...
// Выборка записей (posts) с автором в админку
    function selectAllFromGoodsWithTopics($table1, $table2){
        global $pdo;
        $sql = "SELECT 
        t1.id,
        t1.title,
...
function selectAllFromGoodsWithTopicsOnIndex($table1, $table2){
    global $pdo;
    $sql = "SELECT p.*, u.name FROM $table1 AS p JOIN $table2 AS u ON p.id_topic = u.id";
    $query = $pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetchAll();
...
function seacrhInTitileAndContent($text, $table1, $table2){
    ...
    $sql = "SELECT 
        ...
        JOIN $table2 AS u 
        ON p.id_topic = u.id 
        WHERE  p.title LIKE '%$text%' OR p.content LIKE '%$text%' OR u.name LIKE '%$text%'";
    $query = $pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetchAll();
...
function selectPostFromPostsWithUsersOnSingle($table1, $table2, $id){
    global $pdo;
    $sql = "SELECT p.*, u.name FROM $table1 AS p JOIN $table2 AS u ON p.id_topic = u.id WHERE p.id=$id";
    $query = $pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetch();
